...
#if defined(NDEBUG)
# error "Yocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x41b82d6025c6e2444173a217b33bd341bd34b15398deb6769d0f5d9eb48c6d3e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yocoin Signed Message:\n";
...
{
    // Yocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1471969207, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "23 AUGUST 2016";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0444d392b85aaac660751a92360189453c5bbf78ff092234e88b0a6e9e68095a7506957560754de4ceafd2ca6bf690262aceb53481de1e3e21ac56903cf84a9131") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1471969207;
        ...
        assert(block.hashMerkleRoot == uint256("0x732dc882d7b07c6eb21be507eed254bfe9a70479230e9796df7e7862756ba25b"));
...
unsigned char pchMessageStart[4] = { 0x56, 0x00, 0xda, 0x78 }; // Yocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YocoinMiner(CWallet *pwallet)
{
    printf("YocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yocoin-miner");
    ...
    try { loop {
        ...
        printf("Running YocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YocoinMiner, pwallet));
}
